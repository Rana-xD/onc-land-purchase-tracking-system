    ...
    const handleNext = async () => {
        ...
        try {
            ...
            if (editMode && documentId) {
                // In edit mode, use existing document ID
                currentDocumentId = documentId;
            } else {
                // In create mode, create new document first
                const createEndpoint = `/api/${apiPrefix}/create`;
                const createResponse = await axios.post(createEndpoint, {
                    document_type: documentType,
                });
                currentDocumentId = createResponse.data.id;
            }

            // Add/update the selected buyers
            await axios.post(
                `/api/${apiPrefix}/${currentDocumentId}/select-buyers`,
                {
                    buyer_ids: selected,
                },
            );
            ...
            if (documentType === "deposit_contract") {
                window.location.href = route(
                    "deposit-contracts.select-sellers",
                    { id: currentDocumentId },
                );
            } else {
                window.location.href = route("sale-contracts.select-sellers", {
                    id: currentDocumentId,
                });
            }
        } catch (error) {
            console.error(
                "Error creating document or selecting buyers:",
                error,
            );
            ...
            <div className="container mx-auto py-6">
                ...
                <Card>
                    <div className="flex justify-between items-center mb-6">
                        <Title level={3}>
                            {editMode ? "កែសម្រួលអ្នកទិញ" : "ជ្រើសរើសអ្នកទិញ"}
                        </Title>
                    ...
                    <Table
                        ...
                        rowClassName={(record) =>
                            selected.includes(record.id) ? "bg-blue-50" : ""
                        }
                    />
                    ...
                    <div className="flex justify-between mt-6">
                        <Button
                            href={
                                documentType === "deposit_contract"
                                    ? route("deposit-contracts.index")
                                    : route("sale-contracts.index")
                            }
                        >
                            ត្រឡប់
                        </Button>
